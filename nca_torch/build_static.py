...
def load_instrument_embeddings(spectrograms_dir: Path, encoder, device):
    """Load pre-generated spectrograms and encode them to embeddings.

    Expects directory structure: spectrograms_dir/{instrument}/{note}.png
    Generated by generate_audio_visual.py alongside training data.
    """
    import cv2
    ...
    # Get encoder's expected input channels (context_frames * in_channels)
    context_frames = getattr(encoder, 'context_frames', 1)
    in_channels = getattr(encoder, 'in_channels', 3)
    expected_channels = context_frames * in_channels

    def prepare_input(img):
        """Convert RGB image to encoder's expected input format."""
        # img is [H, W, 3] uint8
        spec_tensor = torch.from_numpy(img).float() / 255.0
        spec_tensor = spec_tensor.permute(2, 0, 1)  # [3, H, W]
        # Tile to match expected channels (e.g., 4 context frames = tile 4x)
        if expected_channels > 3:
            repeats = (expected_channels + 2) // 3  # Ceiling division
            spec_tensor = spec_tensor.repeat(repeats, 1, 1)[:expected_channels]
        return spec_tensor.unsqueeze(0).to(device)

    if not spectrograms_dir.exists():
        print(f"  No spectrograms directory found at {spectrograms_dir}")
        return embeddings

    # Each subdirectory is an instrument
    for instrument_dir in sorted(spectrograms_dir.iterdir()):
        if not instrument_dir.is_dir():
            continue

        instrument_name = instrument_dir.name
        embeddings[instrument_name] = {}

        for note in NOTE_ORDER:
            img_path = instrument_dir / f"{note}.png"
            if not img_path.exists():
                continue

            try:
                # Load image (cv2 loads as BGR, convert to RGB)
                ...
                with torch.no_grad():
                    spec_tensor = prepare_input(img)
                    result = encoder(spec_tensor)
                    # Handle both Encoder (1 output) and VAEEncoder (3 outputs)
                    mu = result[1] if isinstance(result, tuple) else result
                    embedding = mu.squeeze(0).cpu().numpy().tolist()
                embeddings[instrument_name][note] = embedding
            except Exception as e:
                print(f"  Warning: Failed to load {img_path}: {e}")

        if embeddings[instrument_name]:
            print(f"  {instrument_name}: {len(embeddings[instrument_name])} notes")
        else:
            del embeddings[instrument_name]
...
def main():
    ...
    parser.add_argument("--data", type=str, required=True,
                        help="Path to data directory (contains sequences and spectrograms/)")
